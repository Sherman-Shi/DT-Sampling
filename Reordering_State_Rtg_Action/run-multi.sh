...
environments=("halfcheetah-expert-v2" "hopper-medium-v2" "hopper-expert-v2")
...
do
    ...
    run_experiment "DT.py" $env $train_seed_dt $eval_seed_dt &
    
    # Wait for 100 seconds before starting DT-SAR.py
    sleep 100
    
    # Run DT-SAR.py in parallel for the current environment
    run_experiment "DT-SAR.py" $env $train_seed_dt_sar $eval_seed_dt_sar &
